...
import { bossBattleNarration, bossWinNarration, CORRECT_NARRATIONS, WRONG_NARRATIONS } from '../../utils/narration.js';
...
export default function BossBattle({ questions, world, worldAccent, onComplete, playNarration }) {
  ...
  const boss = world?.boss ?? { name: 'Money Boss', emoji: '👑', reward: 'Money Master Badge 👑' };
  ...
  const handleConfirm = () => {
    ...
    if (isCorrect) {
      ...
      playNarration([narr]);
      const gained = XP_REWARDS.CORRECT * 3; // boss mode multiplier
      setXp((x) => x + gained);
      setCorrect((c) => c + 1);
  ...
  if (!started) {
    return (
      <div className="boss-banner glass-card" style={{ maxWidth: 600 }}>
        <motion.div
        ...
        <div className="conversion-fact-box" style={{ marginBottom: 20 }}>
          🔑 Remember: <span className="mass-value">Change = Amount Paid − Price</span>
        </div>
  ...
  return (
    <div style={{ position: 'relative', width: '100%', maxWidth: 600 }}>
      ...
      <FeedbackOverlay
        ...
        message={selected === question?.correctAnswer ? "That's exactly right! Well done! 🎉" : "Not quite! Look at the prices and coins."}
