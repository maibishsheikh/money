...
export function XPPopup({ popups }) {
  return (
    <>
      {popups.map(p => (
        <div key={p.id} className="xp-popup" style={{ left: p.x || '50%', top: p.y || '40%' }}>
          +{p.amount} XP
        </div>
      ))}
    </>
...
export default function GuidedPractice({ questions, worldAccent, onComplete, playNarration }) {
  ...
  const handleDismiss = () => {
    ...
    if (isLast) { 
      onComplete?.({ xp, correct, total: questions.length }); 
      return; 
    }
    setIndex(i => i + 1); 
    setSelected(null); 
